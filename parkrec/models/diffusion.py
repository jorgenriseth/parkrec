...
class Model(object):
    ...
    def save_predictions(
        ...
        hdf5file = HDF5File(
            self.V.mesh().mpi_comm(), str(self.outfolder / "simulation.hdf"), "w"
        )
        ...
        for idx, sim in enumerate(self.simulated_tracer):
            sim.rename("simulation", "simulation")
            hdf5file.write(
                sim, "simulation" + format(self.checkpoints[idx] / 3600, ".0f")
            )

        hdf5file.close()

        checkpoints = {"simulation": self.checkpoints, "data": self.times}
    ...
    def advance_time(self, current_state):
        ...
        if np.round(self.t, 0) in self.checkpoints:
            dt = (
                self.t
                - self.times[np.argmin(np.abs(np.array(self.checkpoints) - self.t))]
            )
    ...
    def boundary_condition(self):
        ...
        finite_difference /= (
            self.times[self.image_counter] - self.times[self.image_counter_prev]
        )

        self.linear_interpolation.vector()[:] += finite_difference * (
            self.t - self.times[self.image_counter_prev]
        )
        ...
        def diffusion(fun):
            if self.dx_SD is not None:
                ...
                if self.diffusion_tensor is not None:
                    ...
                    term += inner(
                        dot(self.diffusion_tensor, grad(fun)), grad(v)
                    ) * self.dx_SD(2)
                else:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--data",
        default="/home/basti/Dropbox (UiO)/Sleep/228/FIGURES_CONC_LUT/",
    ...
    parser.add_argument(
        "--mesh",
        default="/home/basti/Dropbox (UiO)/Sleep/228/mesh/parenchyma8_with_DTI.h5",
    ...
    try:
        ...
        print(
            "No DTI found, using D=1.3e-3 mm^2/s, taken from https://pubmed.ncbi.nlm.nih.gov/32514105/"
        )
    ...
    mris = MRI_Measurements(
        datapath=datapath, function_space=V, Tmax=tmax, file_suffix="_concentration"
    )
